...
function create_heatmap(fig; args...)
#    obsx = Observable(Float64[])
#    obsy = Observable(Float64[])
#    obsz = Observable(Float64[])
    obsx = Observable([0.0])
    obsy = Observable([0.0])
    obsz = Observable([0.0])
    
    ax, hm = heatmap(fig, obsx, obsy, obsz)
...
function create_histogram(fig, label; args...)
    obsable = Observable([0.0])
    
    axis, _ = hist(fig, obsable; label=label, args...)
    axislegend(axis)
